...
const ARCHETYPES = ['formula', 'muscle', 'truck', 'van', 'buggy', 'retro', 'mini', 'hyper', 'wagon', 'pod'];
...
const wheel = (x, y, r) => `<circle cx='${x}' cy='${y}' r='${r}' fill='#111827'/><circle cx='${x}' cy='${y}' r='${Math.max(2, r - 4)}' fill='#9ca3af'/>`;
const lights = (fx, fy, rx, ry) => `<rect x='${fx}' y='${fy}' width='7' height='4' rx='2' fill='#fca5a5'/><rect x='${rx}' y='${ry}' width='9' height='5' rx='2' fill='#fde68a'/>`;
const stripe = (variant, accent) => {
    if (variant % 4 === 0)
        return `<rect x='30' y='29' width='48' height='3' rx='2' fill='${accent}' opacity='.9'/>`;
    if (variant % 4 === 1)
        return `<rect x='36' y='22' width='6' height='14' fill='${accent}' opacity='.86'/><rect x='47' y='22' width='6' height='14' fill='${accent}' opacity='.86'/>`;
    if (variant % 4 === 2)
        return `<polygon points='28,30 52,22 62,22 38,30' fill='${accent}' opacity='.82'/>`;
    return `<polygon points='28,24 62,24 74,30 28,30' fill='${accent}' opacity='.78'/>`;
};
const buildShape = (arch, variant, base, trim, accent) => {
    if (arch === 'formula') {
        return {
...
const carPrice = (index) => 90 + (index % 10) * 18 + Math.floor(index / 10) * 14;
const carRarity = (index) => {
    if (index % 9 === 0)
        return 'epic';
    if (index % 3 === 0)
        return 'rare';
...
export const GENERATED_CAR_DESIGNS = Array.from({ length: 100 }, (_, i) => {
...
export const GENERATED_CAR_MAP = Object.fromEntries(GENERATED_CAR_DESIGNS.map((car) => [car.id, car]));
export const getCarSpriteSrc = (carId) => {
    if (STOCK_CAR_ASSETS[carId])
        return STOCK_CAR_ASSETS[carId];
