...
import type { ClientToHostMessage, DiscoveryPacket, HostConfig, HostToClientMessage } from '../shared/network.js';
import type { LobbyState, MultiplayerConfig, PlayerState, Question } from '../shared/types.js';
...
type RendererEvent =
  ...
  | { type: 'score'; players: PlayerState[]; latest?: { playerId: string; delta: number; correct: boolean } }
  | { type: 'result'; players: PlayerState[]; winnerId?: string }
...
interface HostRuntime {
  ...
  weakFacts: Record<string, { attempts: number; correct: number; averageMs: number; lastSeen: number }>;
...
const discoveredHosts = new Map<string, DiscoveryPacket & { seenAt: number }>();
...
const randomRoomCode = (): string => Math.random().toString(36).slice(2, 7).toUpperCase();

const safeSendWs = (ws: WebSocket, payload: HostToClientMessage): void => {
  if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(payload));
};

const broadcastLobby = (): void => {
  if (!hostRuntime) return;
  const lobbyMessage: HostToClientMessage = { type: 'lobby', lobby: hostRuntime.lobby };
  for (const ws of hostRuntime.clients.values()) safeSendWs(ws, lobbyMessage);
  sendEvent({ type: 'lobby', lobby: hostRuntime.lobby });
};

const stopHost = (): void => {
  if (!hostRuntime) return;
  if (hostRuntime.ticker) clearInterval(hostRuntime.ticker);
  if (hostRuntime.announceTicker) clearInterval(hostRuntime.announceTicker);
...
const startGameIfReady = (): void => {
  ...
  hostRuntime.questionQueue = Array.from({ length: total }).map(() =>
    generateQuestion({
  ...
  const countdownMessage: HostToClientMessage = { type: 'countdown', startsAt };
  for (const ws of hostRuntime.clients.values()) safeSendWs(ws, countdownMessage);
  ...
  const nextQuestion = (): void => {
    ...
    if (now >= hostRuntime.gameEndsAt || hostRuntime.questionIndex >= total) {
      ...
      const result: HostToClientMessage = { type: 'result', winnerId: winner?.id, players: hostRuntime.lobby.players };
      for (const ws of hostRuntime.clients.values()) safeSendWs(ws, result);
    ...
    const q =
      hostRuntime.questionQueue[hostRuntime.questionIndex] ??
      generateQuestion({
        config: hostRuntime.config.config.difficulty,
        weakFacts: hostRuntime.weakFacts,
    ...
    for (const ws of hostRuntime.clients.values()) safeSendWs(ws, msg);
...
const applyAnswer = (playerId: string, payload: { questionId: string; value: number; responseMs: number }): void => {
  if (!hostRuntime || !hostRuntime.currentQuestion || !hostRuntime.lobby.started) return;
  ...
  const delta = calculateAnswerPoints(
    correct,
    payload.responseMs,
    player.streak,
    { ...DEFAULT_SETTINGS, basePoints: 50 },
    hostRuntime.config.config.difficulty.wrongPenalty
  );
  player.score += delta;
  ...
  const scoreMsg: HostToClientMessage = {
    type: 'score',
    players: hostRuntime.lobby.players,
    latest: { playerId, delta, correct }
  };
  for (const ws of hostRuntime.clients.values()) safeSendWs(ws, scoreMsg);
  sendEvent({ type: 'score', players: hostRuntime.lobby.players, latest: { playerId, delta, correct } });
};

const startHost = async (
  roomPartial: Pick<HostConfig, 'hostName' | 'mode' | 'maxPlayers' | 'config'> & { wsPort?: number; discoveryPort?: number }
): Promise<{ roomCode: string; hostIp: string }> => {
  stopHost();
  ...
  wsServer.on('connection', (socket) => {
    socket.on('message', (raw) => {
      ...
      try {
        msg = JSON.parse(raw.toString()) as ClientToHostMessage;
      } catch {
        return;
      }
      ...
      if (msg.type === 'answer') {
        const pid = hostRuntime.sockets.get(socket);
        if (pid) applyAnswer(pid, msg);
      }

      if (msg.type === 'chat') {
        const pid = hostRuntime.sockets.get(socket);
        if (!pid) return;
        const event: HostToClientMessage = { type: 'chat', playerId: pid, text: msg.text.slice(0, 120), at: Date.now() };
        for (const ws of hostRuntime.clients.values()) safeSendWs(ws, event);
        sendEvent(event);
      }
...
const startDiscovery = (port = 41234): void => {
  ...
  discoverySocket.on('message', (message) => {
    try {
      const packet = JSON.parse(message.toString()) as DiscoveryPacket;
  ...
  discoveryPoller = setInterval(() => {
    ...
    for (const [key, host] of discoveredHosts) {
      if (now - host.seenAt > 4000) discoveredHosts.delete(key);
    }
...
const joinRoom = async (hostIp: string, wsPort: number, name: string, color: string): Promise<void> => {
  stopClient();
  await new Promise<void>((resolve, reject) => {
    ...
    ws.on('open', () => {
      ws.send(JSON.stringify({ type: 'join', name, color } satisfies ClientToHostMessage));
      resolve();
    });
    ...
    ws.on('message', (raw) => {
      ...
      try {
        msg = JSON.parse(raw.toString()) as HostToClientMessage;
      } catch {
        return;
      }
      if (msg.type === 'welcome') {
      ...
      if (msg.type === 'lobby') sendEvent({ type: 'lobby', lobby: msg.lobby });
      if (msg.type === 'countdown') sendEvent(msg);
      if (msg.type === 'question') sendEvent(msg);
      if (msg.type === 'score') sendEvent(msg);
      if (msg.type === 'result') sendEvent(msg);
      if (msg.type === 'chat') sendEvent(msg);
      if (msg.type === 'error') sendEvent(msg);
    });
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('net:start-discovery', (_, port?: number) => startDiscovery(port));
  ...
  ipcMain.handle('net:ready', (_, ready: boolean) => {
    if (clientRuntime.ws && clientRuntime.ws.readyState === WebSocket.OPEN) {
      clientRuntime.ws.send(JSON.stringify({ type: 'ready', ready } satisfies ClientToHostMessage));
    }
    if (hostRuntime) {
      const hostPlayer = hostRuntime.lobby.players.find((p) => p.id === 'host-local');
      if (hostPlayer) hostPlayer.ready = ready;
      broadcastLobby();
    }
  });
  ipcMain.handle('net:start-game', () => startGameIfReady());
  ipcMain.handle('net:submit-answer', (_, payload: { questionId: string; value: number; responseMs: number }) => {
    if (clientRuntime.ws && clientRuntime.ws.readyState === WebSocket.OPEN) {
      clientRuntime.ws.send(JSON.stringify({ type: 'answer', ...payload, submittedAt: Date.now() } satisfies ClientToHostMessage));
    }
    if (hostRuntime) {
      applyAnswer('host-local', payload);
    }
  });
  ipcMain.handle('net:chat', (_, text: string) => {
    if (clientRuntime.ws && clientRuntime.ws.readyState === WebSocket.OPEN) {
      clientRuntime.ws.send(JSON.stringify({ type: 'chat', text } satisfies ClientToHostMessage));
    }
  });
