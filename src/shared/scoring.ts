import type { AnswerEvent, RoundStats, Settings } from './types.js';
import { clamp } from './utils.js';

export const calculateAnswerPoints = (
  isCorrect: boolean,
  responseMs: number,
  streak: number,
  settings: Settings,
  ...
  if (!isCorrect) return -wrongPenalty;
...
export const summarizeRound = (
  answers: Array<AnswerEvent & { correct: boolean }>,
  bestStreak: number,
  totalScore: number
): RoundStats => {
  ...
  const averageResponseMs =
    answers.length > 0 ? answers.reduce((sum, ans) => sum + ans.responseMs, 0) / answers.length : 0;

  let streak = 0;
  for (let i = answers.length - 1; i >= 0; i -= 1) {
    if (answers[i].correct) streak += 1;
    else break;
  }
