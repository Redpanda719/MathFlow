import type { DifficultyConfig, FactStatsMap, Question } from './types.js';
import { createSeededRng, factKey, randomPickWith, weightedRandomWith, type RNG } from './utils.js';

export interface GeneratorOptions {
  config: DifficultyConfig;
  weakFacts: FactStatsMap;
  recentQuestionIds: string[];
  mode: 'mixed' | 'focus' | 'adaptive';
  rng?: RNG;
}

export type PromptPool = 'multiplication' | 'division' | 'fractions' | 'sequence' | 'geometry';

export interface MathPrompt {
  id: string;
  prompt: string;
  answer: number;
  meta?: { a?: number; b?: number; explanation?: string };
}

const sanitizeTables = (tables: number[]): number[] => {
...
const allPairs = (tables: number[]): Array<[number, number]> => {
  const set = new Set<number>(sanitizeTables(tables));
  const values = [...set].sort((a, b) => a - b);
  const maxFactor = Math.max(1, ...values);
  const pairs: Array<[number, number]> = [];
  for (const a of values) {
    for (let b = 1; b <= maxFactor; b += 1) pairs.push([a, b]);
  }
  return pairs;
};

const pairWeight = (a: number, b: number, weakFacts: FactStatsMap): number => {
...
export const generateQuestion = ({ config, weakFacts, recentQuestionIds, mode, rng = Math.random }: GeneratorOptions): Question => {
  ...
  if (mode === 'adaptive' || config.adaptive) {
    pair = weightedRandomWith(
      selectionPool.map(([a, b]) => ({
        item: [a, b] as [number, number],
...
export const questionHint = (a: number, b: number): { repeatedAddition: string; groups: string } => ({
...
export const buildSeededPromptStream = (
  seed: number,
  count: number,
  pool: PromptPool,
  difficulty: DifficultyConfig
  ...
  for (let i = 0; i < count; i += 1) prompts.push(generatePrompt(pool, difficulty, rng, i));
  return prompts;
};
