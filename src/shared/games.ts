...
export const GAME_LIBRARY: GameLibraryItem[] = [
  {
    ...
    description:
      'Nitro Type style racing with multiplication prompts, AI racers, and LAN races with synchronized countdowns and live scoreboards.',
    rules: [
  ...
  },
  {
    ...
    description:
      'Place towers on build pads, manage waves, and trigger energy abilities. Math fuels your special actions while towers auto-fight.',
    rules: [
...
export const GAME_BY_ID = Object.fromEntries(GAME_LIBRARY.map((game) => [game.id, game])) as Record<
  LibraryGameId,
  GameLibraryItem
>;
